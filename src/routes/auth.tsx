...
function AuthPage() {
  ...
  return <main className="grid min-h-screen bg-background lg:grid-cols-[.8fr_1.2fr]">
    <section className="flex flex-col border-r border-border px-5 py-6 sm:px-10 lg:px-14">
      ...
      <div className="my-10 w-full max-w-md self-center py-8 sm:my-auto sm:py-12">
        ...
        <form className="mt-8 space-y-4" onSubmit={submit}>
          ...
          <div><div className="mb-2 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3"><label className="min-w-0 text-sm font-semibold" htmlFor="password">Password</label>{mode === "signin" && <Link to="/forgot-password" className="shrink-0 py-1 text-xs font-semibold text-primary">Forgot password?</Link>}</div><div className="relative"><Input id="password" type={showPassword ? "text" : "password"} minLength={8} required value={password} onChange={(e) => setPassword(e.target.value)} className="h-12 bg-card pr-12" placeholder="At least 8 characters" /><Button type="button" size="icon" variant="ghost" className="absolute right-0 top-0 size-12" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff /> : <Eye />}</Button></div></div>
